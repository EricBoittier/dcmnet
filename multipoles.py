...
def load_data(testdata: Path):
    ...
    # grid_idx = np.where(np.all(cdist(grid, xyz) >= (3.0 - 1e-10), axis=-1))[0]
    # grid = grid[grid_idx]
    # esp = esp[grid_idx]
    # grid_idx = np.where(np.all(cdist(grid, xyz) >= (3.5 - 1e-10), axis=-1))[0]
    ...
    return {"xyz": xyz, "atom_types": atom_types, "grid": grid, "esp": esp,
            "monopoles": monopoles,
            "dipoles": dipoles, "quadrupoles": quadrupoles}
...
def plot_moments(data):
    ...
    axs[0].imshow(np.repeat(data["monopoles"], 9, axis=1),
                  cmap='bwr', vmin=-1, vmax=1)
    print(data["monopoles"].shape)
    axs[0].set_title("$l=0$")
    axs[1].imshow(np.repeat(data["dipoles"], 3, axis=1),
                  cmap='bwr', vmin=-1, vmax=1)
    print(data["dipoles"].shape)
    axs[1].set_title("$l=1$")
    print(data["quadrupoles"].shape)
    axs[2].imshow(data["quadrupoles"].reshape(-1, 9),
                  cmap='bwr', vmin=-1, vmax=1)
    axs[2].set_title("$l=2$")
    # set wspace
    for i in range(3):
        axs[i].set_xticks([])
        axs[i].set_yticks([])
    axs[0].set_yticks(range(len(data["atom_types"].tolist())),
...
def plot_esp_slices(data):
    ...
    axs[0].scatter(data["grid"][:, 0], data["grid"][:, 1], c=data["esp"],
                   vmin=-0.05, vmax=0.05, s=1)
    axs[0].set_title("XY Slice")
    axs[1].scatter(data["grid"][:, 0], data["grid"][:, 2], c=data["esp"],
                   vmin=-0.05, vmax=0.05, s=1)
    axs[1].set_title("XZ Slice")
    axs[2].scatter(data["grid"][:, 1], data["grid"][:, 2], c=data["esp"],
                   vmin=-0.05, vmax=0.05, s=1)
...
testdata = Path("/home/boittier/Documents/phd/pythonProject/data/ala-ala_0_0")
files = list(
    Path("/Users/ericboittier/Documents/github/blender_labexp/data").glob("ala-ala_*"))[::5]

testdata = files[-1]
data = load_data(testdata)

plot_esp_slices(data)
...
# plot_esp(data["esp"], quad_esp, title="Quadrupole")
plot_esp(data["esp"], monopole_esp + dipole_esp + quad_esp, title="Monopole + Dipole + Quadrupole")
