...
def plot_3d(grid, esp, atoms=None):
    ...
    if atoms is not None:
        ...
        plot_atoms(atoms, ax2, rotation=('0x,0y,0z'),
                   scale=0.05)
        ...
        fig.colorbar(ax1.get_children()[0], cax=cbar_ax,
                     label='Electrostatic Potential (a.u.)')

        spec5 = fig.add_gridspec(ncols=2, nrows=3, width_ratios=[1,0.5],
                          height_ratios=[1,1,1])
...
def load_data(testdata: Path,
    ...
    return {"xyz": xyz, "atom_types": atom_types,
            "grid": grid, "esp": esp,
            "monopoles": monopoles,
...
def plot_moments(data):
    ...
    axs[2].imshow(data["quadrupoles"].reshape(data["quadrupoles"].shape[0], 9),
                  cmap='bwr', vmin=-1, vmax=1)
    axs[2].set_title("$l=2$")
    # set wspace
    for i in range(3):
    ...
    axs[0].set_yticklabels(data["atom_types"].tolist())
...
if __name__ == "__main__":
    ...
    plot_3d(data["grid"], (monopole_esp ) ,
                       atoms=atoms)
    
    plot_3d(data["grid"], (monopole_esp + dipole_esp ) ,
                       atoms=atoms)
    
    
    plot_3d(data["grid"], (monopole_esp + dipole_esp + quad_esp),
                       atoms=atoms)
